...
Rails.application.configure do
  ...
  config.consider_all_requests_local       = ENV.fetch('RAILS_ALL_REQUESTS_LOCAL', 'false').in? %w[1 on t true]
  ...
  # JS is minified by esbuild (see package.json), not by a Ruby compressor.
  ...
  ssl = %w[true yes 1].include?(ENV.fetch('RAILS_HOST_SSL', nil))
