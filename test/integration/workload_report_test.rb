...
class WorkloadReportTest < ActionDispatch::IntegrationTest
  ...
  test 'member detail links are set-up on first page load' do
    find('a[data-toggle-visibility=".employee-8-ordertimes"]').click
  ...
  test 'member detail links are set-up after changing filter' do
    ...
    find('a[data-toggle-visibility=".employee-6-ordertimes"]').click
  ...
  def login
    login_as(:mark, reports_workload_path(start_date: '1.1.2006', end_date: '31.12.2006', department_id: departments(:devtwo).id))
  end
end
