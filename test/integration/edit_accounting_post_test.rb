...
class EditAccountingPostTest < ActionDispatch::IntegrationTest
  ...
  test "calculate correct budget values" do
    ...
    assert_equal 200.5 / Settings.must_hours_per_day, find_field('accounting_post_offered_days').value.to_f
    assert_equal 200.5 * 7.4, find_field('accounting_post_offered_total').value.to_f

    fill_in('accounting_post_offered_days', with: 77)
    assert_equal 77 * Settings.must_hours_per_day, find_field('accounting_post_offered_hours').value.to_f
    assert_equal 77 * Settings.must_hours_per_day * 7.4, find_field('accounting_post_offered_total').value.to_f

    fill_in('accounting_post_offered_rate', with: 1000)
    assert_equal 77 * Settings.must_hours_per_day * 1000, find_field('accounting_post_offered_total').value.to_f
  ...
  def login
    login_as(:mark, edit_accounting_post_path(accounting_post))
  end
