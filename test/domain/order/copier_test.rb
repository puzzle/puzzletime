...
class Order
  class CopierTest < ActiveSupport::TestCase
    ...
    test 'copies most direct attributes' do
      ...
      assert_equal OrderStatus.defaults.pick(:id), copy.status_id
    ...
    test 'copy of closed order will be open' do
      ...
      assert order.work_item.children.first.closed
      ...
      assert_equal OrderStatus.defaults.pick(:id), copy.status_id
      assert_not copy.work_item.children.first.reload.closed
    end
