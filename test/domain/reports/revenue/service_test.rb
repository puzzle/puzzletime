...
class ServiceRevenueReportTest < ActiveSupport::TestCase
  ...
  test 'entries and values' do
    ...
    assert_equal({ [software.id, Date.new(2000, 7, 1)] => 6.0,
    ...
    assert_in_delta(9.0, r.total_ordertime_hours_per_entry(software))
    assert_in_delta(170.0, r.total_ordertime_hours_per_entry(system))
    assert_in_delta(4.5, r.average_ordertime_hours_per_entry(software))
    assert_in_delta(170.0, r.average_ordertime_hours_per_entry(system))
    assert_in_delta(179.0, r.total_ordertime_hours_overall)
    assert_in_delta(89.5, r.average_ordertime_hours_overall)
    assert_equal({ [system.id, Date.new(2000, 9, 1)] => 6.4 * 170.0,
  ...
  test 'entries and values from configured company are ignored' do
    ...
    assert_equal 0, r.total_ordertime_hours_overall
    assert_equal 0, r.average_ordertime_hours_overall
    assert_equal({ [software.id, Date.new(2000, 11, 1)] => 6.4 * 140.0 }, r.planning_hours)
