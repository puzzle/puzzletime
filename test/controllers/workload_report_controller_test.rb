...
class WorkloadReportControllerTest < ActionController::TestCase
  ...
  test 'GET index without params sets default period' do
    travel_to Time.zone.now.at_noon do
      get :index
      period = assigns(:report).period

      assert_equal(Time.zone.today.prev_month.beginning_of_month, period.start_date)
      assert_equal(Time.zone.today.prev_month.end_of_month, period.end_date)
    end
  end
