...
class MonthEndCompletionsControllerTest < ActionController::TestCase

  setup :login

  def test_edit_as_manager
    order = orders(:puzzletime)
    order.update!(completed_month_end_at: Date.new(2015, 8, 31))
    get :edit, order_id: order.id
    assert_template '_form'

    selection = assigns(:completion_dates)
  ...
  def test_update
    ...
    patch :update,
          order_id: order.id,
          order: { completed_month_end_at: eom }
    assert_equal eom, order.reload.completed_month_end_at
  end
  ...
  def test_update_is_not_allowed_with_arbitrary_dates
    ...
    patch :update,
          order_id: order.id,
          order: { completed_month_end_at: Date.new(2015, 10, 15) }
    assert_equal Date.new(2015, 8, 31), order.reload.completed_month_end_at
    assert_template '_form'
