...
class OrderServicesControllerTest < ActionController::TestCase
  ...
  test 'GET show filtered by period open end' do
    get :show, order_id: order.id, start_date: '10.12.2006'
    assert_equal [worktimes(:wt_lw_puzzletime)], assigns(:worktimes)
  end

  test 'GET show filtered by period open start' do
    get :show, order_id: order.id, end_date: '10.12.2006'
    assert_equal worktimes(:wt_pz_puzzletime, :wt_mw_puzzletime), assigns(:worktimes)
  end

  test 'GET show filtered by period with start and end' do
    get :show, order_id: order.id, start_date: '5.12.2006', end_date: '10.12.2006'
  ...
  test 'GET show filtered by period and employee' do
    get :show, order_id: order.id, end_date: '10.12.2006', employee_id: employees(:pascal).id
    assert_equal [worktimes(:wt_pz_puzzletime)], assigns(:worktimes)
  end

  test 'GET show filtered by invalid start date' do
    get :show, order_id: order.id, start_date: 'abc'
    assert_equal worktimes(:wt_pz_puzzletime, :wt_mw_puzzletime, :wt_lw_puzzletime), assigns(:worktimes)
    assert_match /ungültig/i, flash[:alert]
    assert_equal Period.retrieve(nil, nil), assigns(:period)
  end

  test 'GET show filtered by start date after end date' do
    get :show, order_id: order.id, start_date: '1.12.2006', end_date: '1.1.2006'
    assert_equal worktimes(:wt_pz_puzzletime, :wt_mw_puzzletime, :wt_lw_puzzletime), assigns(:worktimes)
    assert_match /ungültig/i, flash[:alert]
    assert_equal Period.retrieve(nil, nil), assigns(:period)
  end
