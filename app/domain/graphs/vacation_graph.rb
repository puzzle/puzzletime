...
module Graphs
  class VacationGraph
    ...
    def each_employee
      Employee.employed_ones(@actual_period).each do |empl|
        ...
        @monthly_absencetimes = @absences_eval.times(period)
                                              ...
                                              .includes(:absence)
                                              .references(:absence)
                                              .where(report_type: ReportType::HoursMonthType::INSTANCE.key)
    ...
    def add_unpaid_absences(times)
      ...
      @unpaid_absences.each do |unpaid|
        @current.step do |date|
          if unpaid.include?(date) && date.wday.positive? && date.wday < 6
    ...
    def add_monthly(times, period)
      ...
      factor = period.musttime.to_f / month.musttime
      add_absences(times, month, true, factor) if factor.positive?
    end
