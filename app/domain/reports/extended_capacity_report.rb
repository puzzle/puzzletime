...
module Reports
  class ExtendedCapacityReport
    ...
    def employee_summary_row(employee, rows)
      ...
      projekte_total_h = rows.sum { |r| r[12].to_f }
      ...
      [employee.shortname,
       ...
       '',
       rows.sum { |r| r[16].to_f }, # Kunden-Projekte Total (h)
       '',
       rows.sum { |r| r[18].to_f }, # Kunden-Projekte Total verrechenbar (h)
       '',
       rows.sum { |r| r[20].to_f }, # Kunden-Projekte Total nicht verrechenbar (h)
       '',
       rows.sum { |r| r[22].to_f }, # Interne Projekte Total (h)
       '']
    end
