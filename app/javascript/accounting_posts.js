...
$(document).on('turbolinks:load', function () {
  const handleBookOnOrderRadio = function (value) {
    const workItemFields = $('#work_item_fields')
    if (value === 'true') {
      return workItemFields.hide()
    } else {
      return workItemFields.show()
    }
  }

  $('input[name=book_on_order]').on('change', function (event) {
    const value = event.target.value.toLowerCase()
    return handleBookOnOrderRadio(value)
  })

  handleBookOnOrderRadio($('input[name=book_on_order]:checked').val())
