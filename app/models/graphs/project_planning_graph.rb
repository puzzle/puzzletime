...
class ProjectPlanningGraph
  ...
  def initialize(project, period = nil)
    ...
    @cache = {}
    @plannings       = Planning.all(conditions: ['project_id = ? and start_week <= ? and is_abstract=false', @project.id, Week.from_date(period.endDate).to_integer])
    @plannings_abstr = Planning.all(conditions: ['project_id = ? and start_week <= ? and is_abstract=true', @project.id, Week.from_date(period.endDate).to_integer])
